#!/system/bin/sh
# Universal Firewall - Any interface

echo "[*] Detecting interface..."

IFACE=$(cat /data/local/tmp/iotshaper_iface 2>/dev/null)
[ -z "$IFACE" ] && IFACE=$(ip link show | grep -E '(ccmni[0-9]+|rmnet_data[0-9]+|rmnet[0-9]+|eth[0-9]+):' | grep 'UP,LOWER_UP' | grep -v 'wlan\|lo\|dummy\|ifb\|tun\|gre\|sit' | head -1 | awk -F: '{print $2}' | awk '{print $1}')
...
iptables -A OUTPUT -p tcp --dport 5228 -j DROP 2>/dev/null
iptables -A OUTPUT -p tcp --dport 5229 -j DROP 2>/dev/null
